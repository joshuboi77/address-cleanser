...
from src.parser import parse_address, handle_edge_cases
from src.validator import validate_address
from src.formatter import create_formatted_address_result
from src.utils import setup_logging, ensure_directory_exists, validate_file_extension, calculate_processing_stats
...
@click.option('--format', '-f', default='csv', 
              ...
              help='Output format')
@click.option('--address-column', '-c', default='address', 
              help='Name of the address column in input CSV')
...
def batch(ctx, input, output, format, address_column, report, chunk_size):
    ...
    try:
        ...
        logger.info(f"Processing complete. {stats['total_processed']} addresses processed, "
...
@click.option('--format', '-f', default='json', 
              type=click.Choice(['csv', 'json', 'excel']),
              help='Output format')
@click.option('--output', '-o', help='Output file path (optional, prints to console if not specified)')
@click.pass_context
...
def process_csv_file(input_file: str, address_column: str, chunk_size: int, logger) -> List[Dict[str, Any]]:
    """
    ...
    if address_column not in df.columns:
        logger.error(f"Address column '{address_column}' not found in CSV. Available columns: {list(df.columns)}")
...
def write_excel_output(results: List[Dict[str, Any]], output_path: str, logger) -> None:
    ...
    with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
        ...
        stats = calculate_processing_stats(results)
        summary_data = {
            'Metric': ['Total Processed', 'Successful', 'Failed', 'Success Rate (%)', 'Average Confidence'],
            'Value': [stats['total_processed'], stats['successful'], stats['failed'], 
                     stats['success_rate'], stats['average_confidence']]
        }
