...
def validate_zip_code(zip_code: str) -> Tuple[bool, str]:
    ...
    return (
        False,
        f"Invalid ZIP code format: {zip_code}. Expected 5 digits or ZIP+4 format (12345-6789)",
    )
...
def validate_address(address_dict: Dict[str, Any]) -> Dict[str, Any]:
    ...
    confidence = calculate_confidence_score(
        address_dict,
        {"zip_valid": zip_valid, "state_valid": state_valid, "is_complete": is_complete},
    )
...
def validate_street_number(street_number: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        if 1 <= num <= 99999:
            ...
            return (
                False,
                f"Street number out of range: {street_number}. Must be between 1 and 99999",
            )
    except ValueError:
