...
if [[ "$OS" == "Darwin" ]]; then
    BINARY_NAME="address-cleanser-macos-arm64"
    PLATFORM="macos"
elif [[ "$OS" == "Linux" ]]; then
    if [[ "$ARCH" == "x86_64" ]]; then
...
if [[ ! -f "address-cleanser" ]]; then
    echo "Binary not found after extraction: address-cleanser"
    exit 1
fi

# Install binary
echo "Installing binary to $INSTALL_DIR..."
mv "address-cleanser" "$INSTALL_DIR/address-cleanser"
